...
const useWishlistFrameList = () => {

  const {data:allData, isLoading} = useGetWishlistQuery('frame');
  ...
  const modifiedData = useMemo(() => {
    ...
    const responseData = allData?.data?.map((item:TWishlistFrame) => {
        ...
        return {
            product: item?.frameId?.name,
            purchasePrice: item?.frameId?.purchase,
            salesPrice: item?.frameId?.salesPrice,
...
export default useWishlistFrameList;