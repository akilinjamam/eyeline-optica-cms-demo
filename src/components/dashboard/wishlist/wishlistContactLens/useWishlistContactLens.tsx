...
type TWishlistFrame = {
    salesPrice: any;
    contactLensId:TFrame;
    clicked:number;
}

const useWishlistContactLensList = () => {

  const {data:allData, isLoading} = useGetWishlistQuery('contactLens');
  ...
  const modifiedData = useMemo(() => {
    ...
    const responseData = allData?.data?.map((item:TWishlistFrame) => {
        ...
        return {
            product: item?.contactLensId?.name,
            purchasePrice: item?.contactLensId?.purchasePrice,
            salesPrice: item?.contactLensId?.salesPrice,
...
export default useWishlistContactLensList;