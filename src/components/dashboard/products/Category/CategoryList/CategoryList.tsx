...
const CategoryList = () => {
        ...
        const {actionColumns, columns, filterSummary, filters, page, setPage, paginatedData, search, setSearch, setPaginatedData,filteredData, isLoading, showCheck} = useCategoryList();
        ...
        const defaultColumn = columns?.slice(0,8)
         ...
         // Table data
         const tableData = filteredData.map((clens: ICategory) =>
           dynamicColumns.map(item => clens[item.key as keyof ICategory])
         );
         ...
         // PDF hook
         const { handleDownloadPDF } = usePdfDownloader(tableData, header, "Category-List");
