...
const CustomerList = () => {
        ...
        const { columns, filterSummary, filters, page, setPage, paginatedData, search, setSearch, setPaginatedData,filteredData, isLoading} = useCustomerList();
        ...
        const defaultColumn = columns?.slice(0,8)
         ...
         // Table data
         const tableData = filteredData.map((clens: TCustomer) =>
           dynamicColumns.map(item => clens[item.key as keyof TCustomer])
         );
         ...
         // PDF hook
         const { handleDownloadPDF } = usePdfDownloader(tableData, header, "Customer-List");
