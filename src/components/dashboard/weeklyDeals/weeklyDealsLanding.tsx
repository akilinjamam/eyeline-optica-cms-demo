...
const WeeklyDealsLanding = () => {

  const {role} = useFindUser()

  const productCategories = [
  { title: "Manage Deals", icon: <CalendarDays className="w-8 h-8" />, color: "from-indigo-500 to-blue-500", path: '/dashboard/edit_weeklyDeals', role: role === 'employee' || role === 'admin' || role === 'doctor & admin' || role === 'employee & admin' },
];

  return (
    <div className="w-full h-full p-5 bg-gray-50">
      <div className="lg:h-[100vh] h-[90vh] p-2 overflow-y-scroll hide-scrollbar">
        <h2 className="text-2xl font-bold mb-6 text-gray-800">Weekly Deals</h2>
...
export default WeeklyDealsLanding;
