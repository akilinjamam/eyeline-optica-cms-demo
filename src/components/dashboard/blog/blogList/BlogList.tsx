...
const BlogList = () => {
  ...
  const {columns, setSearch, search, actionColumns, filterSummary, paginatedData, setPaginatedData, filters, page, setPage, filteredDataa, isLoading, showCheck} = useBlogList();

   const defaultColumn = columns?.slice(0,3)
          
  const [dynamicColumns, setDynamicColumns] = useState<TableColumn[]>(defaultColumn);
           
  // Table data
  const tableData = filteredDataa.map((clens: IBlog) =>
  dynamicColumns.map(item => clens[item.key as keyof IBlog])
  );
  ...
  // PDF hook
  const { handleDownloadPDF } = usePdfDownloader(tableData, header, "Blog-List");
