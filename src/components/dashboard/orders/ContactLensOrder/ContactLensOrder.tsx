...
import useContactLensOrder from "./useContactLensOrder";
...
import type { IContactLensSaleInfo } from "../../../../types/interface";
...
const ContactLensOrder = () => {
        ...
        const defaultColumn = columns?.slice(0,8)
         ...
         // Table data
         const tableData = filteredData.map((clens: IContactLensSaleInfo) =>
           dynamicColumns.map(item => clens[item.key as keyof IContactLensSaleInfo])
         );
         ...
         // PDF hook
         const { handleDownloadPDF } = usePdfDownloader(tableData, header, "Only-Contact-Lens-List");
...
export default ContactLensOrder; 
