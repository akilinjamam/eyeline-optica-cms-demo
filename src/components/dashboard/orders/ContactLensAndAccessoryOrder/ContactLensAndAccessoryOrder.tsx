...
import type { IContactLensAccessorySaleInfo } from "../../../../types/interface";
import useConactLensAndAcc from "./useContactLensAndAccessoryOrder";
...
const ContactLensAndAccOrder = () => {
        ...
        const defaultColumn = columns?.slice(0,8)
         ...
         // Table data
         const tableData = filteredData.map((clens: IContactLensAccessorySaleInfo) =>
           dynamicColumns.map(item => clens[item.key as keyof IContactLensAccessorySaleInfo])
         );
         ...
         // PDF hook
         const { handleDownloadPDF } = usePdfDownloader(tableData, header, "C-Lens-and-Accessory-List");
...
export default ContactLensAndAccOrder; 
