...
interface ContactLensAccessoryDetailProps {
  contactLens?: ContactLens | null;
  accessory?: any | null;
}

const ContactLensAccessoryDetail: FC<ContactLensAccessoryDetailProps> = ({
  contactLens,
  accessory,
}) => {
  const isContactLens = !!contactLens;
        ...
        <CardHeader className="grid md:grid-cols-2 gap-6 bg-gradient-to-br from-gray-50 to-white p-6">
          ...
          >
            <img
              src={
                isContactLens
                  ? contactLens?.images?.[0] || "/placeholder-contact-lens.png"
                  : accessory?.images?.[0] || "/placeholder-accessory.png"
              }
              alt={isContactLens ? contactLens?.name || "Contact Lens" : "Accessory"}
          ...
          <div className="flex flex-col justify-between space-y-4">
            ...
            <div>
              ...
              {isAccessory && accessory?.items?.length > 0 && (
            ...
            <>
              <section>
                ...
                <ul className="text-gray-600 grid sm:grid-cols-2 gap-x-8 gap-y-2">
                  <li>Color: {contactLens?.color}</li>
                  <li>Material: {contactLens?.material}</li>
                  <li>Water Content: {contactLens?.waterContent}</li>
                  <li>Diameter: {contactLens?.diameter}mm</li>
                  <li>Base Curve: {contactLens?.baseCurve}</li>
                  <li>Power Range: {contactLens?.powerRange}</li>
                  <li>
                    UV Protection: {contactLens?.uvProtection ? "Yes ☀️" : "No"}
                  </li>
              ...
              {contactLens?.features?.length > 0 && (
                <section>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {contactLens?.features?.map((f:any, i:any) => (
...
export default ContactLensAccessoryDetail;
